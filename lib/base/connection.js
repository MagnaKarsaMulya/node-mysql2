...
class BaseConnection extends EventEmitter {
  ...
  query(sql, values, cb) {
    const query_timer = process.hrtime();
    let cmdQuery;
    ...
    const old_cb = (typeof cmdQuery.onResult === 'function' ? cmdQuery.onResult : null);
    cmdQuery.onResult = (err, rows, fields) => {
      this.emit('query', rawSql, [], process.hrtime(query_timer), err);
      if (old_cb !== null) { old_cb(err, rows, fields); }
    }
