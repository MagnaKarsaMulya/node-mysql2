...
class Connection extends EventEmitter {
  ...
  query(sql, values, cb) {
    ...
    if (sql.constructor === Commands.Query) {
      ...
      let query_timer = process.hrtime(), called = false;
      if (typeof sql === 'string') {
        // query(sql, values, cb)
        if (typeof values != 'function') {
          const new_values = (values !== undefined ? values : []);
          const new_cb = (err, rows, fields) => {
            this.emit('query', sql, new_values, process.hrtime(query_timer), err);
            if (typeof cb == 'function') cb(err, rows, fields);
          }
          cmdQuery = Connection.createQuery(sql, new_values, new_cb, this.config);
          called = true;
        } else {
          // query(sql, cb)
          const new_cb = (err, rows, fields) => {
            this.emit('query', sql, [], process.hrtime(query_timer), err);
            values(err, rows, fields);
          }
          cmdQuery = Connection.createQuery(sql, [], new_cb, this.config);
          called = true;
        }
      } else if (typeof sql === 'object') {
        // query(options, cb)
        if (typeof values === 'function') {
          const new_values = (typeof sql.values != 'undefined' ? sql.values : []);
          const new_cb = (err, rows, fields) => {
            this.emit('query', sql.sql, new_values, process.hrtime(query_timer), err);
            values(err, rows, fields);
          }
          cmdQuery = Connection.createQuery(sql, new_cb, undefined, this.config);
          called = true;
        } else {
          // query(sql, values, cb)
          const new_cb = (err, rows, fields) => {
            this.emit('query', sql.sql, values, process.hrtime(query_timer), err);
            if (typeof cb == 'function') cb(err, rows, fields);
          }
          cmdQuery = Connection.createQuery(sql, values, new_cb, this.config);
          called = true;
        }
      }
