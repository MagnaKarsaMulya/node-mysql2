...
class Connection extends EventEmitter {
  ...
  query(sql, values, cb) {
    ...
    if (sql.constructor === Commands.Query) {
      ...
      let query_timer = process.hrtime(), new_values = false, new_cb = false;
      if (typeof sql === 'object') {
        // query(options, cb)
        if (typeof values === 'function') {
          new_values = (err, rows, fields) => {
            this.emit('query', sql.sql, (typeof sql.values != 'undefined' ? sql.values : []), process.hrtime(query_timer), err);
        ...
        // query(sql, values, cb)
        new_cb = (err, rows, fields) => {
          this.emit('query', sql, values, process.hrtime(query_timer), err);
          cb(err, rows, fields);
        }
      }

      cmdQuery = Connection.createQuery(sql,
        (new_values !== false ? new_values : values),
        (new_cb !== false ? new_cb : cb),
        this.config
      );
    }
